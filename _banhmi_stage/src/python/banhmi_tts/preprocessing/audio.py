...
def continuous_log_f0(f0, times, target_times):
    """Interpolate voiced WORLD observations in log-F0 space.

    The returned contour is defined at every target frame. Voicing remains a
    separate target and must not be inferred from this continuous contour.
    Constant edge extrapolation is intentional and matches ``numpy.interp``.
    """
    import numpy as np
...
def extract_pitch(audio, num_frames: int, config: PreprocessConfig) -> Dict[str, Any]:
    ...
    log_f0_continuous = continuous_log_f0(f0, times, target_times)
    log_f0 = np.zeros(num_frames, dtype=np.float32)
    log_f0[voiced] = log_f0_continuous[voiced]
    f0_hz = np.zeros(num_frames, dtype=np.float32)
    f0_hz[voiced] = np.exp(log_f0[voiced]).astype(np.float32)

    if (
        not np.isfinite(log_f0).all()
        or not np.isfinite(log_f0_continuous).all()
        or not np.isfinite(f0_hz).all()
    ):
    ...
    return {
        ...
        "log_f0_continuous": torch.from_numpy(log_f0_continuous),
