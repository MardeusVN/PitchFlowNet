...
`--devices 2` (or more) requires NCCL, which PyTorch's Windows build does not ship. On Windows, run multi-GPU training from WSL2 (or a Linux box/container) instead — `gloo` is the only available fallback backend and segfaults on CUDA tensors in DDP. Single-GPU (`--devices 1`) works fine natively on Windows.
